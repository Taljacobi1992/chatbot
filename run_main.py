...
PROD_CSV = "chatbot/resources/prod.csv"
...
class BotResources:
    ...
    BOT_ICON_CSS = "[id='covaEyes'] , [id='chatBotIcon']"
    CHAT_FOOTER_AVAILABLE_CSS = "[class='kore-chat-footer']"
    INSERT_TEXT_TO_CHAT_CSS = "[placeholder='Message...'],[aria-placeholder='Message...']"
...
if __name__ == '__main__':
    ...
    result = []
    headers = ['Partner', 'FAQ#', 'PreAuth', 'Correct', 'Expected', 'Actual', 'Notes']
    result.append(headers)
    result_folder = os.path.join(ROOT_DIR, RESULT)
    create_new_folder(result_folder)

    driver = web_driver_init()
    df = create_dataframe_from_csv_file(os.path.join(ROOT_DIR, PROD_CSV))
    ...
    for index, row in df.iterrows():
        ...
        # case:  sign out
        partner_df = create_dataframe_from_csv_file(os.path.join(ROOT_DIR, f'chatbot/resources/sign_out/{partner}.csv'))
        pre_auth = False
        time.sleep(2)
        driver.get(url)
        time.sleep(5)
        for index, faq_row in partner_df.iterrows():
            ...
            screenshot = '' if correct else driver.save_screenshot(os.path.join(result_folder, f'{partner}_{faq_number}.png'))
            if screenshot:
                screenshot = os.path.join(result_folder, f'{partner}_{faq_number}.png')
            result.append([partner, faq_number, pre_auth, correct, expected_utterance, message, screenshot])

            # case:  sign in

        partner_df = create_dataframe_from_csv_file(
            os.path.join(ROOT_DIR, f'chatbot/resources/sign_in/{partner}.csv'))
        pre_auth = True
